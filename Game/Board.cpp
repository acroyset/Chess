...
void Board::set(const Board &other){
...
[[nodiscard]] Piece Board::getPiece(const Position position) const {
...
[[nodiscard]] PieceType Board::getPieceType(const Position position) const {
...
void Board::getValidMoves(MoveList& validMoves) {
    validMoves.clear();
...
void Board::getCaptureMoves(MoveList& validMoves) {
    validMoves.clear();
...
void Board::getValidMoves(const Position position, MoveList& validMoves) {
    validMoves.clear();
...
[[nodiscard]] uint64_t Board::computeHash() const {
    uint64_t hash = 0;
...
[[nodiscard]] uint64_t Board::computePolyglotHash() const {
    uint64_t hash = 0;
...
[[nodiscard]] uint64_t Board::getFriendlyBits() const {
    if (!state.playerTurn) {
...
void Board::clearIncrementalEval() {
    state.whiteMg = 0.0f;
    state.whiteEg = 0.0f;
    state.blackMg = 0.0f;
    state.blackEg = 0.0f;
...
void Board::addPieceToEval(Piece piece, Position position) {
...
void Board::removePieceFromEval(Piece piece, Position position) {
    if (piece == EMPTY || position.isNone()) return;
...
void Board::refreshIncrementalEval() {
...
void Board::movePiece(const Position starting, const Position target, const Piece piece) {
    clear(starting);
...
void Board::set(Position position, Piece piece) {
    clear(position);
    if (piece != EMPTY)
        state.pieceBits[piece] |= 1ULL << position.index;
...
void Board::getPossibleMoves(const Position position, MoveList& moves) const {
    Piece piece = getPiece(position);
...
void Board::getPossibleCaptures(const Position position, MoveList& moves) const {
    Piece piece = getPiece(position);
...
[[nodiscard]] bool Board::check(const bool player) const {
...
[[nodiscard]] bool Board::drawInsufficientMaterial() const {
...
[[nodiscard]] bool Board::squareAttacked(Position square, bool byBlack) const {
...
[[nodiscard]] bool Board::validMove(const Move& move) {
...
[[nodiscard]] bool Board::noMoves(const bool player) {
    for (Position position; position <= Position(7, 7); position++) {
        Piece piece = getPiece(position);
...
[[nodiscard]] Position Board::findKing(bool player) const {
