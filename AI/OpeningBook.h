...
inline std::unordered_map<uint64_t, std::vector<BookMove>> openingBook;
...
inline Move decodePolyglotMove(uint16_t m) {
    int toFile   =  m        & 7;
    int toRank   = (m >> 3)  & 7;
    int fromFile = (m >> 6)  & 7;
    int fromRank = (m >> 9)  & 7;
    ...
    if (fromRank == 0 && fromFile == 4 &&
        toRank == 0 && toFile == 7) {
        return Move{{0,4}, {0,6}, CastleKing};
    }

    if (fromRank == 0 && fromFile == 4 &&
        toRank == 0 && toFile == 0) {
        return Move{{0,4}, {0,2}, CastleQueen};
    }

    if (fromRank == 7 && fromFile == 4 &&
        toRank == 7 && toFile == 7) {
        return Move{{7,4}, {7,6}, CastleKing};
    }

    if (fromRank == 7 && fromFile == 4 &&
        toRank == 7 && toFile == 0) {
        return Move{{7,4}, {7,2}, CastleQueen};
    }

    return Move{
        Position{fromRank, fromFile},
        Position{toRank, toFile}
    };
}

inline void loadOpeningBook(const std::string& path) {
    std::ifstream file(path, std::ios::binary);

    if (!file.is_open()) {
        throw std::runtime_error("Could not open book");
    }

    RawPolyglotEntry entry{};

    while (file.read(reinterpret_cast<char*>(&entry), sizeof(entry))) {

        uint64_t key = swap64(entry.key);
        uint16_t moveCode = swap16(entry.move);
        uint16_t weight = swap16(entry.weight);

        openingBook[key].push_back({
...
inline std::optional<Move> getBookMove(uint64_t hash) {
...
inline constexpr uint64_t polyglotRandom[781] = {
